...
c Perform a surface energy balance over the domain.
     ...
     &        Tcanopy_adjust,Tcanopy_adjust_flag)
...
c   during the simulation.
     ...
     &          (((real(melt_flag(i,j,k)),i=1,nx),j=1,ny),k=1,nz_max)
          ...
          if (run_snowpack.eq.1.0 .and. multilayer_snowpack.eq.1 .and.
     &      microstructure_flag.eq.1.0) then
            if (mod(iter,iprint_inc).eq.0) then
              write(86,rec=iter/iprint_inc)
     &          (((dendricity(i,j,k),i=1,nx),j=1,ny),k=1,nz_max),
     &          (((sphericity(i,j,k),i=1,nx),j=1,ny),k=1,nz_max),
     &          (((ssa(i,j,k),i=1,nx),j=1,ny),k=1,nz_max),
     &          (((grain_sz(i,j,k),i=1,nx),j=1,ny),k=1,nz_max),
     &          (((d_opt(i,j,k),i=1,nx),j=1,ny),k=1,nz_max),
     &          (((dy_snow(i,j,k),i=1,nx),j=1,ny),k=1,nz_max)
            endif
          endif
