...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     &  svf_ascii_flag,svf_ascii_fname,
     &  microstructure_flag,b92_flag,c13_flag,f06_flag,t07_flag,
     &  microstructure_output_fname,Tcanopy_adjust_flag,snthrm_flag)
...
c Put parameter names here:
     ...
     &  izero_snow_date,albedo_flag
      ...
      integer microstructure_flag,b92_flag,c13_flag,
     & f06_flag,t07_flag,Tcanopy_adjust_flag,snthrm_flag
      
      integer  svf_ascii_flag
...
c GENERAL MODEL SETUP.
     ...
     &        c_param(1:i_param_chars))
          ...
          if (c_param(1:i_param_chars).eq.'svf_ascii_flag') then
            ipar_count = ipar_count + 1
            cpar_name(ipar_count) = c_param(1:i_param_chars)
            call char2int(svf_ascii_flag,i_value_chars,c_value,
     &        c_param(1:i_param_chars))
            if (svf_ascii_flag.ne.0 .and. svf_ascii_flag.ne.1) 
     &       then
              print *,'svf_ascii_flag out of range'
              stop
            endif
          endif        
          if (c_param(1:i_param_chars).eq.'svf_ascii_fname') then
            ipar_count = ipar_count + 1
            cpar_name(ipar_count) = c_param(1:i_param_chars)
            call char2char(svf_ascii_fname,c_value,i_value_chars,
     &        c_param(1:i_param_chars))
          endif
...
c END J.PFLUG
c SWEISS
          if (c_param(1:i_param_chars).eq.'Tcanopy_adjust_flag') then
            ipar_count = ipar_count + 1
            cpar_name(ipar_count) = c_param(1:i_param_chars)
            call char2int(Tcanopy_adjust_flag,i_value_chars,c_value,
     &        c_param(1:i_param_chars))
            if (Tcanopy_adjust_flag .ne.0 .and. Tcanopy_adjust_flag
     &      .ne.1) then
              print *,'Tcanopy_adjust_flag not 0 or 1'
              stop
            endif
          endif
c SWEISS end
...
c SWEISS
          if (c_param(1:i_param_chars).eq.'microstructure_flag') then
            ipar_count = ipar_count + 1
            cpar_name(ipar_count) = c_param(1:i_param_chars)
            call char2int(microstructure_flag,i_value_chars,c_value,
     &        c_param(1:i_param_chars))
            if (microstructure_flag.ne.0 .and. microstructure_flag
     &      .ne.1) then
              print *,'microstructure_flag not 0 or 1'
              stop
            endif
          endif
          if (c_param(1:i_param_chars).eq.'b92_flag') then
            ipar_count = ipar_count + 1
            cpar_name(ipar_count) = c_param(1:i_param_chars)
            call char2int(b92_flag,i_value_chars,c_value,
     &        c_param(1:i_param_chars))
            if (b92_flag.ne.0 .and. b92_flag.ne.1) then
              print *,'b92_flag not 0 or 1'
              stop
            endif
          endif
          if (c_param(1:i_param_chars).eq.'c13_flag') then
            ipar_count = ipar_count + 1
            cpar_name(ipar_count) = c_param(1:i_param_chars)
            call char2int(c13_flag,i_value_chars,c_value,
     &        c_param(1:i_param_chars))
            if (c13_flag.ne.0 .and. c13_flag.ne.1) then
              print *,'c13_flag not 0 or 1'
              stop
            endif
          endif 
          if (c_param(1:i_param_chars).eq.'f06_flag') then
            ipar_count = ipar_count + 1
            cpar_name(ipar_count) = c_param(1:i_param_chars)
            call char2int(f06_flag,i_value_chars,c_value,
     &        c_param(1:i_param_chars))
            if (f06_flag.ne.0 .and. f06_flag.ne.1) then
              print *,'f06_flag not 0 or 1'
              stop
            endif
          endif
          if (c_param(1:i_param_chars).eq.'t07_flag') then
            ipar_count = ipar_count + 1
            cpar_name(ipar_count) = c_param(1:i_param_chars)
            call char2int(t07_flag,i_value_chars,c_value,
     &        c_param(1:i_param_chars))
            if (t07_flag.ne.0 .and. t07_flag.ne.1) then
              print *,'t07_flag not 0 or 1'
              stop
            endif
          endif
          if (c_param(1:i_param_chars).eq.'snthrm_flag') then
            ipar_count = ipar_count + 1
            cpar_name(ipar_count) = c_param(1:i_param_chars)
            call char2int(snthrm_flag,i_value_chars,c_value,
     &        c_param(1:i_param_chars))
            if (snthrm_flag.ne.0 .and. snthrm_flag.ne.1) then
              print *,'snthrm_flag not 0 or 1'
              stop
            endif
          endif
          if (c_param(1:i_param_chars).eq.
     &    'microstructure_output_fname') then
            ipar_count = ipar_count + 1
            cpar_name(ipar_count) = c_param(1:i_param_chars)
            call char2char(microstructure_output_fname,c_value,
     &        i_value_chars,c_param(1:i_param_chars))
          endif
c SWEISS end		 
