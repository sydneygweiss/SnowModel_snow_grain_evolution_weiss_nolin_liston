...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     &  Tcanopy_adjust_flag,svf,Tcanopy_adjust)
     ...
     &  barnes_lg_domain,UTC_flag,wind_lapse_rate,pertPrec
      ...
      real svf(nx_max,ny_max)
      integer Tcanopy_adjust_flag
      real Tcanopy_adjust(nx_max,ny_max)
...
c       print *,'   solving for solar radiation'
     ...
     &    xlon_grid,svf,Tcanopy_adjust_flag)
...
c If requested modify temperature within the canopy based on 
c incoming solar radiation. Webster et al. 2017
      if (Tcanopy_adjust_flag .eq. 1) then 
        call Tcanopy(Qsi_grid,nx,ny,Tair_grid,Tcanopy_adjust,svf)
      endif
...
c       print *,'   solving for incoming longwave radiation'
        if (Tcanopy_adjust_flag .eq. 1) then 
          call longwave(nx,ny,rh_grid,Tcanopy_adjust,Qli_grid,topo,
     &    vegtype,forest_LAI,T_lapse_rate,Td_lapse_rate,
     &    calc_subcanopy_met,cloud_frac_factor,svf, 
     &    Tcanopy_adjust_flag)
        else 
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     &  vegtype,forest_LAI,T_lapse_rate,Td_lapse_rate,
     &  calc_subcanopy_met,cloud_frac_factor,svf,Tcanopy_adjust_flag)
...
c Modify the incoming longwave radiation for the forest canopy.
          if (Tcanopy_adjust_flag .eq. 0) then 
...
c Define the forest-canopy parameters.
     ...
     &            (Stef_Boltz * Tair_grid(i,j)**4) * forest_frac
          ...
          elseif(Tcanopy_adjust_flag .eq. 1) then 
           Qli_grid(i,j) = (svf(i,j)/100 * Qli_grid(i,j)) + ((1 - 
     &     (svf(i,j))/100) * (Stef_Boltz * Tair_grid(i,j)**4))
          endif 
        enddo
      enddo
...
c   direct incoming solar radiation to reach the snow surface.
            ...
            elseif (Tcanopy_adjust_flag .eq. 1) then 
              Qsi_grid(i,j) = svf(i,j)/100 * Qsi_grid(i,j)
            endif 
          endif
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      subroutine Tcanopy(Qsi_grid,nx,ny,Tair_grid,Tcanopy_adjust,svf)
      implicit none 
      include 'snowmodel.inc'
      integer nx       ! number of x output values
      integer ny       ! number of y output values
      ...
      do j=1,ny
        do i=1,nx
          if (svf(i,j) .lt. 90.0) then 
c           if (Qsi_grid(i,j) .lt. 1.0) then 
c             Tcanopy_adjust(i,j) = Tair_grid(i,j) + 3.0 
c           elseif ((Qsi_grid(i,j) .le. 1.0) 
c    &      .and. (Qsi_grid(i,j).lt. 400.0)) then 
c             Tcanopy_adjust(i,j) = Tair_grid(i,j)
c           elseif (Qsi_grid(i,j) .gt. 400.0) then 
c             Tcanopy_adjust(i,j) = 2.63 + 0.88 * Tair_grid(i,j)
c    &        + 0.0073 * Qsi_grid(i,j)
c           endif 
            
            if (Qsi_grid(i,j) .lt. 1.0) then 
              Tcanopy_adjust(i,j) = Tair_grid(i,j) + 1.3 
            elseif ((Qsi_grid(i,j) .ge. 1.0) 
     &      .and. (Qsi_grid(i,j).lt. 400.0)) then 
              Tcanopy_adjust(i,j) = Tair_grid(i,j) 
            elseif (Qsi_grid(i,j) .gt. 400.0) then 
c Option one, where you fit air Temps and SW rad > 400 to 
c corresponding Tcan temps. 
              Tcanopy_adjust(i,j) = -2.37942 + 1.008467 * 
     &           Tair_grid(i,j) - 0.00141 * Qsi_grid(i,j)
c Option two, fit Tcan temps and SW rad > 400 to corresponding Tair
c temps, because this is supposed to represent Edge effects, and 
c Tcan measurements are in the dense forest, so Tcan is more like 
c Tair in this situation. 
c             Tcanopy_adjust(i,j) = 2.97 + 0.99 * Tair_grid(i,j)
c    &        + 0.0014 * Qsi_grid(i,j)
c
c Option three, negate the values in Option 1 
c 
c             Tcanopy_adjust(i,j) = 2.38 + 1.01 * Tair_grid(i,j)
c    &        + 0.0014 * Qsi_grid(i,j)
            endif 
c           print*, Tcanopy_adjust(i,j)
 
          elseif (svf(i,j) .ge. 90.0) then
c           print *,Tair_grid(i,j)
            Tcanopy_adjust(i,j) = Tair_grid(i,j) 
          endif 
        enddo
      enddo 
