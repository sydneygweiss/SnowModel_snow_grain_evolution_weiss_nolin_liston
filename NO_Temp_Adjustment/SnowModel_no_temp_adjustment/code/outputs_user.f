...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     &  seaice_run,print_inc,dendricity,sphericity,d_opt,
     &  grain_sz,ssa,microstructure_flag)
...
c   saving each data arrays to individual files, etc.
     ...
     & sphericity(nx_max,ny_max,nz_max),grain_sz(nx_max,ny_max,nz_max),
     & ssa(nx_max,ny_max,nz_max),d_opt(nx_max,ny_max,nz_max)
      integer l,nz,microstructure_flag,m_vars,m 
...
c Define the output variable data block.
      parameter (m_vars=5)
      real vvars(nx_max,ny_max,nz_max,m_vars)

      character*1 m_var(m_vars)
      character*8 m_outvars(m_vars)

      data m_outvars/'dend_fin','sphr_fin','grsz_fin',
     &              'ssa__fin','dopt_fin'/
...
!sw
cVALUES SAVED FROM MICROSTRUCTURE 
c   21   dend_fin(i,j,l) = dendricity(i,j,l)
c   22   sphr_fin(i,j,l) = sphericity(i,j,l)
c   23   grsz_fin(i,j,l) = grain_sz(i,j,l)
c   24   ssa__fin(i,j,l) = ssa(i,j,l)
c   25   dopt_fin(i,j,l) = d_opt(i,j,l)
...
c 22 = spher_fin(i,j,l) = sphericity(i,j,l)
        m_var(2) = 'y'  
c 23  grain_sz_fin(i,j,l) = grain_sz(i,j,l)
        m_var(3) = 'y'
c 24  ssa_fin(i,j,l) = ssa(i,j,l) 
        m_var(4) = 'n'
c 25  d_opt_fin(i,j,l) = d_opt(i,j,l)  
        m_var(5) = 'y'
      endif 
...
c Open individual output files for each variable.
            if (iter.eq.iter_start) then
              if (icorr_factor_loop.eq.1) then
                do m=1,m_vars
                  if (m_var(m).eq.'y') then
                    open (420+m,file=path1//m_outvars(m)//'.gdat',
     ...
     &                recl=4*nx*ny*nz_max)
              ...
              if (icorr_factor_loop.eq.2) then
                do m=1,m_vars
                  if (m_var(m).eq.'y') then
                    open (520+m,file=path2//m_outvars(m)//'.gdat',
     &                form='unformatted',access='direct',
     &                recl=4*nx*ny*nz_max)
                  endif
                enddo
...
c Initialize the arrays for microstrucutre 
        if (iter.eq.iter_start) then
          if (microstructure_flag .eq. 1) then
            do l=1,nz_max
              do j=1,ny
        ...
        if (microstructure_flag .eq. 1) then 
          do l = 1,nz_max 
            do j=1,ny
               do i=1,nx
c Values saved at the end of the day.
                vvars(i,j,l,1) = dendricity(i,j,l)
                vvars(i,j,l,2) = sphericity(i,j,l)
                vvars(i,j,l,3) = grain_sz(i,j,l)
                vvars(i,j,l,4) = ssa(i,j,l)
                vvars(i,j,l,5) = d_opt(i,j,l)
               enddo
            enddo
          enddo 
        endif 
...
c Write out MICROSTRUCTURE DATA 
        if (microstructure_flag .eq. 1) then 
          if (icorr_factor_loop.eq.1) then
            do m=1,m_vars
              if (m_var(m).eq.'y') then
                write (420+m,rec=irec) (((vvars(i,j,l,m),i=1,nx),j=1,
     &          ny),l=1,nz_max)
              endif
            enddo
          elseif (icorr_factor_loop.eq.2) then
            do m=1,m_vars
              if (m_var(m).eq.'y') then
                write (520+m,rec=irec) (((vvars(i,j,l,m),i=1,nx),j=1,
     &          ny),l=1,nz_max)
              endif
            enddo
          endif
