...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     &  svf_ascii_fname,svf_ascii_flag,svf,
     &  microstructure_output_fname,microstructure_flag,dendricity,
     &  sphericity,ssa,grain_sz,d_opt)
     ...
     &  multilayer_output_fname
      ...
      integer svf_ascii_flag,microstructure_flag
      real svf(nx_max,ny_max)
      character*80 svf_ascii_fname,microstructure_output_fname
...
c Read off the header lines.  I will assume that all of this
c   information was input in the .par file correctly.
      if (svf_ascii_flag .eq. 1)then 
        open (502,file=svf_ascii_fname,form='formatted')
        iheader = 6
        do k=1,iheader
          read (502,*)
        enddo
c Read the data in as real numbers, and do the yrev.
        do j=ny,1,-1
          read (502,*) (svf(i,j),i=1,nx)
        enddo
      endif
c Now that we have read in the svf data arrays, check
c   whether all of the values look like valid numbers.
      do i=1,nx
        do j=1,ny
          if (svf(i,j).lt.0.0 .or. svf(i,j).gt.100.0) then
            print *, 'Found Invalid SVF Value'
            print *, '     Value =',svf(i,j),'  at ',i,j
            stop
          endif
        enddo
      enddo 
...
c   distributions.
     ...
     &  swe_lyr,ro_layer,T_old,gamma,dendricity,sphericity,ssa,
     &  grain_sz,d_opt,microstructure_flag)
...
c    For random precipitation error
      ...
      if (run_snowpack.eq.1.0 .and. multilayer_snowpack.eq.1 .and.
     &  print_multilayer.eq.1.0 .and. microstructure_flag .eq.1) then
        open (86,file=microstructure_output_fname,
     &    form='unformatted',access='direct',
     &    recl=4*(6*nx*ny*nz_max))
      endif
